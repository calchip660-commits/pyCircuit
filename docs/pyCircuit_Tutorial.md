...
> **注意（pyc4.0 硬断点）**：本文档描述的是 pyCircuit v4.0 之前的 **CycleAware** 旧接口（如
> `CycleAwareCircuit` / `CycleAwareDomain` / `domain.next()` / `compile_cycle_aware`）。
> 该旧模型在 pyc4.0（`pyc0.40`）中已不再作为主线接口使用。
>
> 请以以下文档为准（pyc4.0）：
> - `docs/QUICKSTART.md`
...
```python
from pycircuit import CycleAwareCircuit, CycleAwareDomain, compile_cycle_aware
...
def counter(m: CycleAwareCircuit, domain: CycleAwareDomain) -> None:
    ...
    next_count = (count + 1) if enable else count           # 组合逻辑（cycle 0）
...
```
D = (
    exception_entry if exception else
    (branch_target if branch_taken else
     ((pc_r + inst_len) if advance else pc_r))
)
```
...
next_val = (count_r + 1) if enable else count_r          # 组合逻辑，cycle 0
...
```
cycle 0:  count_r (Q) ──→ +1 ──→ 条件选择(enable) ──→ next_val
                                                      │
...
current_pc = boot_pc if is_first else fetch_pc_r
window = build_byte_mem(m, domain, raddr=current_pc, ...)
next_pc = fb_redirect_pc if fb_flush else (current_pc + inst_len)
...
# 数据前递
v = fb_ex_alu if (fwd_ex_ok & (srcl == idex_regdst_r)) else srcl_val
...
# 赋值 ID/EX flop 的 D 端
idex_op_r.set(op_id if id_to_ex_valid else c(OP_INVALID, 6), when=~fb_freeze_id)
idex_regdst_r.set(regdst_id if id_to_ex_valid else c(REG_INVALID, 6), when=~fb_freeze_id)
# ...
